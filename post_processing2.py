...
# --- Configuration (driven by config.json — no hardcoded paths) ---
INPUT_CSV         = config.path("cleaned_csv")
OUTPUT_CSV        = os.path.join(config.root(), "sputtering_database_clean_final.csv")
...
SPUTTERING_KEYWORDS = [
    'sputter',
    'Sputter',      # NOTE: comma was missing here before — caused Python to silently
    'magnetron',    # concatenate 'Sputter' + 'magnetron' = 'Sputtermagnetron' (never matched)
    ...
    'pulsed dc',
    'sputterring',  # Common typo variant
    'Sputterring',
]
...
def filter_sputtering_data():
    ...
    try:
        ...
        print(f"❌ Error: Could not find {INPUT_CSV}. Please check config.json paths.")
        return
    ...
    mask = df['method_lower'].apply(
        lambda x: any(keyword.lower() in x for keyword in SPUTTERING_KEYWORDS)
    )

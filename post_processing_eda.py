...
# === Configuration Paths (driven by config.json — no hardcoded paths) ===
INPUT_DIR  = config.path("extracted_data_dir")
OUTPUT_CSV = config.path("cleaned_csv")

# === Outlier threshold — documented constant, not a magic number ===
THICKNESS_CUTOFF_NM = config.get("vector_db.thickness_outlier_threshold_nm")  # 2000 nm = 2μm
...
axes[1,1].hist(df_plot[df_plot['Thickness_nm'] < THICKNESS_CUTOFF_NM]['Thickness_nm'].dropna(), bins=40, color="#d62728", edgecolor="black")
axes[1,1].set_title(f"Film Thickness (nm) [Filtered < {THICKNESS_CUTOFF_NM/1000:.0f}μm]")
